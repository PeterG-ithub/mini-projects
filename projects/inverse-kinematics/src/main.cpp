...
void updateLegs(Leg& leg1, Leg& leg2, sf::Vector2f& targetPos) 
{
    sf::Vector2f basePos = leg1.position;
    float distance = Calc::distance(basePos, targetPos);
    float l1 = leg1.getLength();
    float l2 = leg2.getLength();
    float len = l1 + l2;
    if (distance < len)
    {
        float x = targetPos.x - basePos.x;
        float y = -(targetPos.y - basePos.y);
